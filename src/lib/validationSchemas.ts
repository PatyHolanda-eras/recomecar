  ...
  .refine(
    (url) => {
      try {
        ...
        // Permite qualquer caractere válido em URLs (letras, números, acentuação, hífens, underscores, pontos, etc)
        // Apenas verifica se tem conteúdo após /in/ ou após /
        return /^\/(?:in\/)?[^\/\s]+\/?$/.test(path);
      } catch {
        return false;
      }
    },
